...
function directionRight(boundingRect) {
  let distance = boundingRect.width;
  return (
    getHorizontal(distance, "INFINITESCROLLRIGHT") +
    `.infinite-scroll-right {position:absolute;left:${-distance}px`
  );
}
...
function directionDown(boundingRect) {
  let distance = boundingRect.height;
  return (
    getVertical(distance, "INFINITESCROLLDOWN") +
    `.infinite-scroll-down {position:absolute;top:${-distance}px`
  );
}
